...
export class ItemRenderer {
  ...
  private async loadItemTextures(
    ...
    for (const itemType of items.keys()) {
      // Try to load texture, use herring texture as fallback for missing enemy textures
      textures[itemType.glId] = Textures.loadFromFile(
        `assets/items/${itemType.texture}`,
        false,
        gl,
      ).catch(() => {
        console.warn(`Missing texture: ${itemType.texture}, using fallback`);
        // Use herring texture as fallback
        return Textures.loadFromFile(
          `assets/items/herring.webp`,
          false,
          gl,
        );
      });
